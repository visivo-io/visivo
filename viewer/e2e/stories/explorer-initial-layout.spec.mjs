 ...
 *
 * All 8 tests start from the same fresh state and don't modify it destructively
 * (except Step 7 which adds an insight), so we run in serial mode with a shared page.
 * Step 7 must run last or near-last since it mutates state.
 *
...
test.describe('Initial Layout -- Right Panel Ordering', () => {
  ...
  test('Step 2: An insight is auto-created on first visit', async () => {
  ...
  test('Step 3: Auto-created insight appears below chart section', async () => {
    const chartSection = page.locator('[data-testid="chart-crud-section"]');
    const insightSections = page.locator('[data-testid^="insight-crud-section-"]');
  ...
  test('Step 4: Auto-created insight is expanded by default', async () => {
  ...
  test('Step 5: Add Insight button is below existing insights', async () => {
  ...
  test('Step 6: First visit has save enabled due to auto-created insight', async () => {
