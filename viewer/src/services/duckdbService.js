...
class DuckDBService {
  ...
  /**
   * Create cohorts from data based on cohort configuration
   * @param {Object} data - Raw trace data
   * @param {string} cohortOn - Column or expression to group by
   * @returns {Promise<Object>} Cohorted data grouped by cohort values
   */
  async createCohorts(data, cohortOn) {
    if (!cohortOn) {
      // No cohort grouping, return data under 'values' key
      return { values: data };
    }
    ...
    try {
      ...
      await this.loadData(tableName, data);

      // Build cohort query
      const columns = Object.keys(data);
      const selectColumns = columns
        .filter(col => col !== 'cohort_on') // Exclude any remaining cohort_on columns
        .map(col => `"${col}"`)
        .join(', ');

      const cohortQuery = `
        SELECT 
          ${cohortOn} as cohort_value,
          ${selectColumns}
        FROM ${tableName}
      ...
      const results = await this.executeQuery(cohortQuery);

      // Group results by cohort value
      const cohortedData = {};
      results.forEach(row => {
        const cohortValue = row.cohort_value;
        delete row.cohort_value; // Remove the cohort column from data
        
        if (!cohortedData[cohortValue]) {
          cohortedData[cohortValue] = {};
          // Initialize arrays for each column
          Object.keys(row).forEach(col => {
            cohortedData[cohortValue][col] = [];
          });
        }
        
        // Add row data to appropriate cohort
        Object.keys(row).forEach(col => {
          cohortedData[cohortValue][col].push(row[col]);
        });
      });
      ...
      try {
        ...
        console.warn('Failed to cleanup temporary table:', cleanupError);
      }
      throw error;
    }
  }
