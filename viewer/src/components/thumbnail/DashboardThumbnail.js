...
function DashboardThumbnail({ dashboard, project, onThumbnailGenerated }) {
  ...
  useEffect(() => {
    const generateThumbnail = async () => {
      if (containerRef.current && chartsLoaded) {
        try {
          ...
          const thumbnail = tempCanvas.toDataURL('image/png', 0.95);
          onThumbnailGenerated(dashboard.name, thumbnail);
        } catch (error) {
          console.error(`[Thumbnail] Error generating for ${dashboard.name}:`, error);
        }
      }
    };

    generateThumbnail();
  }, [dashboard, onThumbnailGenerated, chartsLoaded]);

  const handleChartsLoaded = () => {
