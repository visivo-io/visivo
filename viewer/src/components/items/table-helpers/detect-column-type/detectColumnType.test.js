...
describe('detectColumnType', () => {
  ...
  describe('threshold boundary testing - with larger datasets', () => {
    it('should return numeric when mostly numeric (90%)', () => {
      // Use a larger dataset to make random sampling less flaky
      const data = [
        ...Array.from({ length: 9 }, (_, i) => ({ value: i })), // 9 numeric
        { value: 'text1' },  // 2 text = 90% numeric
      ];

      expect(detectColumnType(data, 'value')).toBe('numeric');
    });

    it('should return text when mostly text (70% numeric)', () => {
      const data = [
        ...Array.from({ length: 7 }, (_, i) => ({ value: i })), // 7 numeric  
        ...Array.from({ length: 3 }, (_, i) => ({ value: `text${i}` })), // 3 text = 70% numeric
      ];
