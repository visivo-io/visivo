  ...
  return (
    <Tooltip content={label} placement="bottom" trigger="hover">
      <Link
        ...
        tabIndex={0}
        className="relative flex items-center justify-center h-12 w-12 text-white hover:text-[#D25946] focus:text-[#D25946] transition-colors duration-200 outline-none"
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        onFocus={() => setFocused(true)}
        onBlur={() => setFocused(false)}
      >
        {/* Icon tumbles up on hover/focus */}
        <span
          className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ease-in-out
            ${hovered || focused ? 'opacity-0 -translate-y-6' : 'opacity-100 translate-y-0'}
          `}
        >
          <Icon className="w-6 h-6" />
        </span>
        {/* Label tumbles down on hover/focus */}
        <span
          className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ease-in-out text-sm font-medium
            ${hovered || focused ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}
          `}
        >
          {label}
        </span>
