...
describe('FieldPill (shared field/column pill)', () => {
  ...
  describe('long labels must truncate, never push trailing controls out of the row', () => {
    ...
    test('the extra slot never shrinks, so a long label cannot squeeze the chevron away', () => {
      ...
      const extraSlot = screen.getByTestId('field-pill-extra');
      expect(extraSlot).toHaveClass('shrink-0');
      expect(within(extraSlot).getByTestId('chev')).toBeInTheDocument();
      // The chevron lives in its own slot, NOT inside the truncating label.
      expect(within(screen.getByText(LONG)).queryByTestId('chev')).not.toBeInTheDocument();
    });
  });
});
