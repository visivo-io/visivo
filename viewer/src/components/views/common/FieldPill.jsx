  ...
  (
    ...
    return (
      ...
      >
        ...
        {extra && (
          // `shrink-0` so a long label can never squeeze the trailing control
          // (e.g. PillMenu's chevron) out of the row; testid'd because the
          // wrapper has no user-facing handle and a test must assert it holds
          // that class without reaching through `.parentElement`.
          <span data-testid="field-pill-extra" className="shrink-0 inline-flex items-center">
            {extra}
          </span>
        )}
        {children}
