...
export const WorksheetProvider = ({ children }) => {
  ...
  useEffect(() => {
    const loadInitialState = async () => {
      setIsLoading(true);
      try {
        const [worksheetData] = await Promise.all([listWorksheets()]);
  ...
  const handleUpdateWorksheet = useCallback(
    async (worksheetId, updates) => {
      try {
        ...
        setWorksheets(prev => {
          ...
          return prev.map(w => (w.id === worksheetId ? { ...w, ...updates } : w));
        });
  ...
  const handleReorderWorksheets = useCallback(
    async newOrder => {
      try {
        const updatedSessionState = sessionState.map(state => {
