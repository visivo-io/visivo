 ...
 * @param {Object} input - Input job object to validate
 * @returns {boolean} True if valid
 */
const validateInputJobStructure = input => {
  if (!input.name) {
    return false;
  }
...
/**
 * Fetch input jobs metadata with retry logic
 * @param {string} projectId - Project ID
 * @param {string[]} names - Array of input names
 * @param {number} retries - Number of retries (default 3)
 * @param {number} retryDelay - Delay between retries in ms (default 1000)
 * @returns {Promise<Object[]>} Array of input job metadata objects
 */
export const fetchInputJobs = async (projectId, names, retries = 3, retryDelay = 1000) => {
  // In server mode, this will call /api/input-jobs/ with input names as query params
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to fetch input jobs data: ${response.status} ${response.statusText}. ${errorText}`
        );
      }
      ...
      // Validate each input
      const validInputs = inputs.filter(input => validateInputJobStructure(input));
 ...
 * @typedef {Object} InputJobMetadata
...
/**
 * Load input job data from JSON file URL
 *
