...
def test_create_worksheet(repository):
    ...
    result = repository.create_worksheet(
        name="Test Worksheet", query="SELECT * FROM test", selected_source="test_source"
    )
...
def test_get_worksheet(repository):
    ...
    repository.save_results(
        worksheet_id, results_json='{"data": "test"}', query_stats_json='{"stats": "test"}'
    )
...
def test_update_worksheet(repository):
    ...
    success = repository.update_worksheet(
        worksheet_id,
        {"name": "Updated Name", "query": "SELECT * FROM updated", "selected_source": "new_source"},
    )
...
def test_save_results_nonexistent_worksheet(repository):
    ...
    success = repository.save_results(
        "nonexistent-id", results_json='{"data": "test"}', query_stats_json='{"stats": "test"}'
    )
    assert success is False
