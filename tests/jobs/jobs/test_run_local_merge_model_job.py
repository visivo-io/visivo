...
def test_job_with_csv_script_model():
    ...
    csv_model = CsvScriptModelFactory(
        name="csv_model", args=["echo", "col1,col2\n1,2\n3,4"]
    )
    ...
    lmm_job = job(
        dag=project.dag(), local_merge_model=local_merge_model, output_dir=output_dir
    )
...
def test_job_with_nested_local_merge_model():
    ...
    lmm_job = job(
        dag=project.dag(), local_merge_model=outer_merge_model, output_dir=output_dir
    )
