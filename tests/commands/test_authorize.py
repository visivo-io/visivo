...
def dummy_run_server(monkeypatch):
    ...
    monkeypatch.setattr(
        "visivo.commands.authorize.run_flask_server", lambda **kwargs: None
    )
...
def mock_file_writes(monkeypatch):
    ...
    """
    monkeypatch.setattr(
        "visivo.tokens.server.validate_and_store_token", lambda token: None
    )
...
def test_authorize_timeout_cancel(monkeypatch):
    ...
    result = runner.invoke(
        authorize, ["--host", "http://localhost:3030"], input="y\ny\n"
    )
