...
MODEL_DEFS = {
    'HeightEnum':{'enum': ['small', 'medium', 'large'], 'title': 'HeightEnum', 'type': 'string'},
...
def test_handle_attribute_properties_anyOf():
    anyOf_property = {'anyOf': [{'pattern': '^ref\\(\\s*([a-zA-Z0-9\\s\'\\"\\-_]+)\\)$',
    'type': 'string'},
   {'$ref': '#/$defs/Model'}],
...
def test_handle_attribute_properties_items():
  ...
  'type': 'array'}
    ...
    assert description == 'A list of items containing tables, charts or markdown. Items are placed in the row in the order that they are listed from left to right.'
    assert default == 'None'
...
def test_handle_attribute_properties_discriminator():
    ...
    type, description, default = markdown.handle_attribute_properties(MODEL_DEFS, discriminator_property)
    print(type)
