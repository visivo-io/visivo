...
class TestTelemetryEvents:
    ...
    def test_cli_event_basic(self):
        ...
        event = CLIEvent.create(
            command="run", command_args=["--dag-filter", "+chart1+"], duration_ms=1234, success=True
        )
        ...
        assert event.properties["command_args"] == ["--dag-filter", "+chart1+"]
        ...
        assert "platform_version" in event.properties
        assert "architecture" in event.properties
    ...
    def test_cli_event_to_dict(self):
        ...
        event = CLIEvent.create(
            command="serve", command_args=["--port", "<value>"], duration_ms=100, success=True
        )
    ...
    def test_api_event_basic(self):
        ...
        event = APIEvent.create(
            endpoint="/api/query/{project_id}", method="POST", status_code=200, duration_ms=150
        )
