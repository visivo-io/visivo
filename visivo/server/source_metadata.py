...
def get_database_schemas(sources, source_name, database_name):
    ...
    try:
        ...
        with engine.connect() as conn:
            new_inspector = _switch_database_context(
                conn, engine, database_name, src.database
            )
            if new_inspector:
        ...
        return _build_error_response(
            source_name, e, database=database_name
        ), 500
...
def get_schema_tables(sources, source_name, database_name, schema_name=None):
    ...
    try:
        ...
        return _build_error_response(
            source_name, e, database=database_name, schema=schema_name
        ), 500
