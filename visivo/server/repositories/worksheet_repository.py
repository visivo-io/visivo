...
class WorksheetRepository:
    ...
    def create_worksheet(self, name: str, query: str = "", selected_source: str = None):
        ...
        try:
            # Get max tab order
            max_order = session.query(SessionStateModel.tab_order).order_by(
                SessionStateModel.tab_order.desc()
            ).first()
            ...
            worksheet.session_state = SessionStateModel(
    ...
    def list_worksheets(self):
        ...
        try:
            ...
            return [{
                "worksheet": w.to_dict(),
                "session_state": w.session_state.to_dict()
            } for w in worksheets]
        finally:
            session.close()
    ...
    def update_session_states(self, states: list):
        ...
        try:
            for state in states:
                session_state = session.query(SessionStateModel).filter_by(
                    worksheet_id=state['worksheet_id']
                ).first()
                if session_state:
