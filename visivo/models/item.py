...
class Item(BaseModel):
    ...
    ```
    """
    width: int = Field(1, description="The width of the Item determines is evaluated relative to the other items in a row.")
    markdown: Optional[str] = Field(None, description="Markdown text to include in the dashboard.")
    chart: Optional[Union[constr(regex=REF_REGEX), Chart]] = Field(None, description="A chart object defined inline or a ref() to a chart.")
    table: Optional[Union[constr(regex=REF_REGEX), Table]] = Field(None, description="A Table object defined inline or a ref() to a table")
