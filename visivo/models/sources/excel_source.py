...
class ExcelFileSource(BaseSource):
    ...
    def get_model_schema(self, model_sql: str = None, table_name: str = None) -> Dict[str, str]:
        """Extract column metadata from Excel source using DuckDB in-memory connection.

        Excel sources create a view with the source name, so we query that directly.
        """
        # For Excel sources, we typically query the view created with the source name
