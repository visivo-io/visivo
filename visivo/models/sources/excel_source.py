...
class ExcelFileSource(Source):
    ...
    def get_schema(self, table_names: List[str] = None) -> Dict[str, Any]:
        """
        Build SQLGlot schema for this Excel source.

        Args:
            table_names: Optional list of table names to include. For Excel sources,
        ...
        try:
            ...
            # Read sample of Excel to infer schema
            try:
            ...
            # Build schema for the Excel "table"
            table_schema = {
                "columns": {},
                "metadata": {
            ...
            for col_name in df_sample.columns:
                ...
                table_schema["columns"][col_name] = {
                    ...
                    "nullable": True,  # Excel columns are generally nullable
            ...
            Logger.instance().debug(
                f"Built Excel schema for '{self.name}' with {len(df_sample.columns)} columns"
            )

            return result

        except Exception as e:
            Logger.instance().error(f"Error building schema for Excel source {self.name}: {e}")
            return {
                "tables": {},
                "sqlglot_schema": MappingSchema(),
