...
class Dimensions1(TracePropsAttribute):
    ...
    displayindex: Optional[int | IndexedStatementField] = Field(
        None,
...
class Parcats(TraceProps):
    ...
    bundlecolors: Optional[bool | IndexedStatementField] = Field(
        None,
        description=""" boolean<br>Sort paths so that like colors are bundled together within each category. """,
    )
    counts: Optional[IndexedStatementField | StatementField | float | List[float]] = (
        Field(
            None,
    ...
    dimensions: Optional[List[Dimensions1]] = Field(
        None,
        description=""" array of object where each object has one or more of the keys listed below.<br> """,
    )
    ...
    type: Literal["parcats"] = Field(..., description=""" "parcats"<br> """)
