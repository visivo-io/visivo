...
class Image(TraceProps):
    ...
    dx: Optional[float | constr(pattern=INDEXED_STATEMENT_REGEX)] = Field(
        None, description=""" number<br>Set the pixel's horizontal size. """
    )
    dy: Optional[float | constr(pattern=INDEXED_STATEMENT_REGEX)] = Field(
        None, description=""" number<br>Set the pixel's vertical size """
    )
    ...
    type: Literal["image"] = Field(..., description=""" "image"<br> """)
