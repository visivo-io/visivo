...
class Cluster1(TracePropsAttribute):
    color: Optional[str | List[str]] = Field(
        None, description=""" color or array of colors<br>Sets the color for each cluster step. """
    )
    enabled: Optional[bool | constr(pattern=INDEXED_STATEMENT_REGEX)] = Field(
        None, description=""" boolean<br>Determines whether clustering is enabled or disabled. """
    )
...
class ScattermapboxMarker(TracePropsAttribute):
    allowoverlap: Optional[bool | constr(pattern=INDEXED_STATEMENT_REGEX)] = Field(
        None, description=""" boolean<br>Flag to draw all symbols, even if they overlap. """
    )
    angle: Optional[
        constr(pattern=INDEXED_STATEMENT_REGEX)
        | constr(pattern=STATEMENT_REGEX)
...
class Scattermapbox(TraceProps):
    ...
    lat: Optional[constr(pattern=STATEMENT_REGEX) | List] = Field(
        None, description=""" data array<br>Sets the latitude coordinates (in degrees North). """
    )
    ...
    lon: Optional[constr(pattern=STATEMENT_REGEX) | List] = Field(
        None, description=""" data array<br>Sets the longitude coordinates (in degrees East). """
    )
