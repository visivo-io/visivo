...
class CarpetAaxis(TracePropsAttribute):
    ...
    gridcolor: Optional[str] = Field(None, description=""" color<br>Sets the axis line color. """)
...
class Carpet(TraceProps):
    ...
    da: Optional[float | constr(pattern=INDEXED_STATEMENT_REGEX)] = Field(
        None, description=""" number<br>Sets the a coordinate step. See `a0` for more info. """
    )
    db: Optional[float | constr(pattern=INDEXED_STATEMENT_REGEX)] = Field(
        None, description=""" number<br>Sets the b coordinate step. See `b0` for more info. """
    )
    font: Optional[
        TextfontLabelfontTickfontInsidetextfontRangefontOutsidetextfontFontGrouptitlefont1
    ] = Field(
