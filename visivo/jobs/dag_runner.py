...
class DagRunner:
    ...
    def _extract_all_dimensions(self):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(self._extract_dimensions_for_model, model): model
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    Logger.instance().debug(
                        f"Failed to extract dimensions for {model.name}: {str(e)}"
                    )
    ...
    def _extract_dimensions_for_model(self, model):
        ...
        try:
            source = get_source_for_model(
                model=model, dag=self.project_dag, output_dir=self.output_dir
            )
            if source:
